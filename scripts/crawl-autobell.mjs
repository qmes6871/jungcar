...
async function main() {
  console.log('Starting autobell.co.kr crawler with detailed info...');
  ...
  try {
    ...
    const loginLink = await page.$('a[href*="login"], a:has-text("로그인"), .login, .btn_login');
    if (loginLink) {
    ...
    // Fill login form
    const userSelectors = ['#user_id', '#userId', '#mb_id', '#id', 'input[name="mb_id"]', 'input[name="user_id"]'];
    const passSelectors = ['#user_pw', '#password', '#mb_password', 'input[type="password"]'];
    ...
    if (userField && passField) {
      ...
      const loginBtn = await page.$('button:has-text("로그인"), input[type="submit"], button[type="submit"]');
      if (loginBtn) {
    ...
    console.log('Looking for car list...');
    const buyCarMenu = await page.$('a:has-text("내차사기")');
    if (buyCarMenu) {
      await buyCarMenu.click();
      await page.waitForTimeout(3000);
    } else {
      const urls = [
        'https://autobell.co.kr/buycar',
        'https://autobell.co.kr/buycar/buyCarList',
        'https://autobell.co.kr/usedcar',
      ];
      for (const url of urls) {
        try {
          await page.goto(url, { waitUntil: 'networkidle', timeout: 15000 });
          const title = await page.title();
          if (!title.includes('찾을 수 없') && !title.includes('404')) break;
        } catch (e) {
          continue;
        }
      }
    ...
    const cars = await page.evaluate((brandMapArg) => {
      const results = [];
      const carCards = document.querySelectorAll('[class*="CarCard"], [class*="carCard"], .card-item, .product-card, article');
      ...
      if (elements.length < 3) {
        ...
        elements = Array.from(allDivs).filter(div => {
          ...
          const hasBrand = text.includes('현대') || text.includes('기아') || text.includes('제네시스') ||
                          text.includes('BMW') || text.includes('벤츠');
    ...
    for (let i = 0; i < cars.length; i++) {
      ...
      if (brand !== 'Unknown' && (price > 0 || mileage > 0)) {
        processedCars.push({
          externalId: `autobell-${Date.now()}-${i}`,
          brand,
    ...
    if (uniqueCars.length > 0) {
      ...
      const deleted = await prisma.auctionCar.deleteMany({
        where: { externalId: { startsWith: 'autobell-' } }
      });
      console.log(`Cleared ${deleted.count} old autobell entries`);
    ...
    await page.screenshot({ path: '/tmp/autobell-error.png' });
  } finally {
    await browser.close();
