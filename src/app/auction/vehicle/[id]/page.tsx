...
export default function SsancarCarDetailPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCarData() {
      try {
        ...
        const res = await fetch(`${baseUrl}/data/vehicle-auction.json`);
        if (res.ok) {
  ...
  const getImageUrl = (path: string) => {
    if (!path) return '/images/cars/default.jpg';
            ...
            >
              ...
              <div className="rounded-2xl bg-[#0a4d0e] p-6 text-white shadow-lg">
                ...
                <p className="mt-3 text-sm text-white/80">
                  This vehicle is available at our partner auction. We can assist you with:
                </p>
          ...
          >
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
              {relatedCars.map((relCar) => (
                <Link key={relCar.id} href={`/auction/vehicle/${relCar.id}`}>
