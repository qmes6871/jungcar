...
async function main() {
  ...
  console.log('오토허브 옥션 전체 상세정보 크롤러 시작');
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      ...
      console.log(`    ${carIds.length}대 차량 발견`);
      ...
      for (let i = 0; i < carIds.length; i++) {
        ...
        try {
          ...
          // 상세 정보 추출
          const carDetail = await page.evaluate(() => {
            const text = document.body.innerText;
            const detail = {};
          ...
          console.log(`    에러 (${carId}): ${err.message}`);
          // 오류 시 목록으로 재이동
          await page.evaluate(() => { checkAuthority('AC1'); });
      ...
      if (pageNum < totalPages) {
        try {
          ...
          console.log(`    페이지 이동 에러: ${e.message}`);
          // 재시도
          await page.evaluate(() => { checkAuthority('AC1'); });
          await page.waitForTimeout(3000);
          await page.evaluate((np) => { CmPageMove(String(np)); }, pageNum + 1);
      ...
      // 중간 저장 (50페이지마다)
      if (pageNum % 50 === 0) {
        saveResults(allCars, `autohub-cars-partial-${pageNum}.json`);
        console.log(`    중간 저장 완료: ${allCars.length}대`);
      }
    }
    ...
    // 최종 저장
    saveResults(allCars, 'autohub-cars.json');

  } catch (error) {
    console.error('크롤링 에러:', error.message);
    await page.screenshot({ path: '/tmp/autohub-full-error.png' });
    // 에러 발생시에도 수집된 데이터 저장
    if (allCars.length > 0) {
      saveResults(allCars, 'autohub-cars.json');
...
function saveResults(cars, filename) {
...
function extractBrand(name) {
  ...
  const brands = {
    ...
    '미니': 'Mini', '인피니티': 'Infiniti', 'KG모빌리티': 'KG Mobility',
  };
...
function extractModel(name) {
  if (!name) return '';
  return name.replace(/^(현대|기아|제네시스|벤츠|BMW|아우디|폭스바겐|도요타|혼다|렉서스|닛산|볼보|쉐보레|쌍용|르노|포드|지프|테슬라|랜드로버|재규어|포르쉐|링컨|캐딜락|마세라티|미니|인피니티|KG모빌리티)\s*/, '').trim();
}
